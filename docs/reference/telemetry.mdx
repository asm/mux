...
Alternatively, set `XUM_DISABLE_TELEMETRY` before starting the app:

```bash
XUM_DISABLE_TELEMETRY=1 xum
```

The environment variable is a hard override: when set, telemetry stays off regardless of the Settings toggle. Both switches disable collection at the backend level.
