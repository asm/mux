...
export const BUILTIN_SKILL_FILES: Record<string, Record<string, string>> = {
  ...
  "xum-docs": {
    ...
    "references/docs/config/server-access.mdx": [
      ...
      "",
      "Downloads install an exact package version in a sibling staging directory without changing the running installation. Restart is blocked by active streams, pending turns, workspaces still initializing or being archived, removed, renamed, forked, or staged, workflow runs, project clones and creations, any other request still in flight, queued messages, pending auto-retries, open or starting terminals, live desktop sessions, and running processes without recoverable monitors. Armed background watchers with verified, persisted monitor registrations do not block restart. After restart, their workspaces receive a lost-monitor wake and can relaunch them as needed; shell commands are not replayed automatically. Finish or stop any blocking work, then retry. There is no automatic restart-when-idle in this version.",
      "",
