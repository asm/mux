...
export const BUILTIN_SKILL_FILES: Record<string, Record<string, string>> = {
  ...
  "xum-docs": {
    ...
    "references/docs/hooks/tools.mdx": [
      ...
      "",
      "<details>",
      "<summary>session_history (13)</summary>",
      "",
      "| Env var                             | JSON path            | Type    | Description |",
      "| ----------------------------------- | -------------------- | ------- | ----------- |",
      "| `XUM_TOOL_INPUT_ACTION`             | `action`             | enum    | —           |",
      "| `XUM_TOOL_INPUT_CURSOR`             | `cursor`             | string  | —           |",
    ...
    "references/docs/workspaces/compaction/token-budget.md": [
      ...
      "- A **final flush** (the **Context window ending: notes flush** row) is offered when a settled tool step crosses the rollover point mid-stream with enough headroom for one more notes-writing step. The agent gets a single provider step for one `memory` call, with every tool other than `memory` disabled for that step (history retrieval belongs to the next window), `memory` limited to one create-or-update of `workspace/context-notes.md`, only that file visible in the memory index and preload, its output bounded to a notes-sized cap, and repository tool hooks, programmatic tool calling, and MCP server startup turned off; the window is then sealed regardless of what it wrote. The flush step is housekeeping: like compaction, it does not count toward a goal's turn or cost caps. The final flush is skipped when memory is read-only, `session_history` is unavailable, the projected usage leaves no headroom (checked again when the step dispatches), a user message is already queued, the rollover happens on a new user message rather than mid-stream, or Xum restarted before the flush step dispatched.",
      "",
      "The prompts report usage against the usable budget (the **Rolls over by N%** point, or the hard request ceiling if lower), not the model's full context window.",
      "",
