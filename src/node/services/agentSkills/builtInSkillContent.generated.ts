...
export const BUILTIN_SKILL_FILES: Record<string, Record<string, string>> = {
  ...
  "xum-docs": {
    ...
    "references/docs/adr/0005-token-budget-context-windows.md": [
      "---",
    ...
    "references/docs/adr/0006-agent-led-context-handoff.md": [
      "---",
    ...
    "references/docs/docs.json": [
      ...
      '              "adr/0005-token-budget-context-windows",',
      '              "adr/0006-agent-led-context-handoff",',
      '              "AGENTS"',
      "            ]",
      "          }",
      "        ]",
    ...
    "references/docs/hooks/tools.mdx": [
      ...
      "| `XUM_TOOL_INPUT_MONITOR_MAX_EVENTS`     | `monitor.max_events`     | number  | Stop monitoring after this many matching lines; the process keeps running. A monitor retired this way also stops watching for process settlement (no exit wake).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |",
      "| `XUM_TOOL_INPUT_MONITOR_WAKE_ON_EXIT`   | `monitor.wake_on_exit`   | boolean | Also wake when the monitored process settles (exit, kill, timeout), even if no line ever matched. Explicit cancellation (task_stop, terminate without flush, workspace cleanup) and max_events retirement produce no settlement wake. Defaults to true.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |",
      ...
      "",
      "<details>",
      "<summary>session_history (12)</summary>",
      "",
      "| Env var                             | JSON path            | Type    | Description |",
      "| ----------------------------------- | -------------------- | ------- | ----------- |",
      "| `XUM_TOOL_INPUT_ACTION`             | `action`             | enum    | —           |",
      "| `XUM_TOOL_INPUT_ITEM_ID`            | `item_id`            | string  | —           |",
      "| `XUM_TOOL_INPUT_LIMIT`              | `limit`              | number  | —           |",
      "| `XUM_TOOL_INPUT_LIMIT_CHARS`        | `limit_chars`        | number  | —           |",
      "| `XUM_TOOL_INPUT_MAX_CHARS_PER_ITEM` | `max_chars_per_item` | number  | —           |",
      "| `XUM_TOOL_INPUT_OFFSET_CHARS`       | `offset_chars`       | number  | —           |",
      "| `XUM_TOOL_INPUT_QUERY`              | `query`              | string  | —           |",
      "| `XUM_TOOL_INPUT_RECENT_FIRST`       | `recent_first`       | boolean | —           |",
      "| `XUM_TOOL_INPUT_ROLE`               | `role`               | enum    | —           |",
      "| `XUM_TOOL_INPUT_TASK_ID`            | `task_id`            | string  | —           |",
      "| `XUM_TOOL_INPUT_TOOL_NAME`          | `tool_name`          | string  | —           |",
      "| `XUM_TOOL_INPUT_WINDOW_ID`          | `window_id`          | string  | —           |",
      "",
      "</details>",
      "",
      "<details>",
      ...
      "",
      "<details>",
      "<summary>workflow_run (5)</summary>",
      "",
      "| Env var                            | JSON path           | Type    | Description                                                                                                                                                                                                                                                                                                                                                              |",
      "| ---------------------------------- | ------------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |",
      "| `XUM_TOOL_INPUT_ALLOW_CONCURRENT`  | `allow_concurrent`  | boolean | Pass true only to intentionally start another active run of the same script in this workspace. By default workflow_run refuses when the same script already has an active (pending/running/backgrounded) run and reports that run so you can task_await or workflow_resume it instead of duplicating it.                                                                 |",
    ...
    "references/docs/workspaces/compaction/token-budget.md": [
      "---",
      "title: Token-Budget Context Windows",
